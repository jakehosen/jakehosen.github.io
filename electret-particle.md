...
## Particle Boron version of the code
```
/*
 * boron_soundscape_ndvi_lp.cpp
 *
 * Particle BORON (nRF52840 + cellular modem) acoustic band-energy logger,
 * duty-cycled with ULTRA_LOW_POWER sleep.
 *
 * Analog electret mic (MAX4466, fixed gain) -> A5 -> blocking micros()-paced
 * sampler -> Hann-windowed FFT -> Welch-averaged power spectrum -> octave
 * bands -> published to the existing "ndvi-vals" webhook.
 *
 * Requires: Device OS 2.0.0 or later (ULTRA_LOW_POWER sleep mode)
 *           NO external libraries. Delete SparkIntervalTimer from lib/ if it
 *           is still in the project -- it is Gen 1/Gen 2 only (Core/Photon)
 *           and will not compile for Gen 3 at any version.
 *
 * ---------------------------------------------------------------------------
 * WHAT CHANGED FROM THE ARGON VERSION
 *   The nRF52840, ADC, FFT, and band math are identical. Networking differs:
 *   Cellular instead of WiFi, a much longer connect timeout, Cellular.RSSI()
 *   for signal strength, NETWORK_INTERFACE_CELLULAR for sleep standby.
 *
 *   One behavioral change beyond the API swap: by default the modem stays OFF
 *   during sampling and is only powered up to publish (RADIO_OFF_WHILE_SAMPLING
 *   below). A cellular modem's transmit bursts pull far more current than an
 *   ESP32's and couple into the analog mic line as broadband noise. That noise
 *   is amplitude, not timing, so the SAMPLE_RATE_TOLERANCE check will NOT
 *   catch it -- contaminated windows would average in looking perfectly valid.
 *   Sampling with the radio down removes the problem instead of detecting it,
 *   and saves the power of holding a connection while capturing audio.
 *
 *   Consequence: in duty-cycle mode this publishes ONCE per wake, not five
 *   times. Cycling the modem between publishes would cost 30+ seconds of
 *   reconnection each time.
 *
 * WHY ULTRA_LOW_POWER AND NOT HIBERNATE
 *   On Gen 3, HIBERNATE cannot wake on a timer -- only on a pin. ULP is the
 *   only mode supporting timed wake, and it resumes execution on the line
 *   after System.sleep() with all variables intact.
 *
 *   ULP powers down the ADC and GPIO, so pinMode() is re-asserted on wake.
 * ---------------------------------------------------------------------------
 *
 * WEBHOOK
 *   Publishes to "ndvi-vals". The webhook's secret_key lives in the Particle
 *   console integration, NOT in this firmware.
 *
 * Band values are dB relative to ADC full scale (dBFS) -- RELATIVE, not SPL.
 */
...
// System thread is enabled by default in Device OS 6.2.0 and later.
// Uncomment if building against an older Device OS:
// SYSTEM_THREAD(ENABLED);
...
// Set false to run continuously (wall power / bench testing). In continuous
// mode the modem stays connected and a spectrum publishes every
// REPORT_INTERVAL_MS, matching the old Argon behavior.
const bool ENABLE_SLEEP_CYCLE = true;

// Upper bound on how long the device stays awake each cycle. In duty-cycle
// mode the device normally sleeps as soon as it has sampled and published --
// this is a safety cap for when the modem cannot connect, not the usual path.
// Must exceed MIC_WARMUP_MS + sampling time + CONNECT_TIMEOUT.
const unsigned long AWAKE_TIME_MS = 5UL * 60UL * 1000UL;    // 5 minutes
...
// Keep the modem off while capturing audio; power it up only to publish.
// Strongly recommended on cellular -- see the header note.
const bool RADIO_OFF_WHILE_SAMPLING = true;

// Keep the modem in standby through sleep for a faster reconnect. Costs real
// current, and is ignored when RADIO_OFF_WHILE_SAMPLING is true (the two are
// mutually exclusive by design). Only worth it for sleeps of a few minutes.
const bool KEEP_MODEM_IN_STANDBY = false;
...
// --- Averaging (Welch's method) -------------------------------------------
// Windows averaged into one reported spectrum. 31 x 64 ms ~= 2.0 s.
// On cellular this is now the WHOLE measurement for a wake cycle, so consider
// raising it substantially -- 470 gives ~30 s of integration per publish at
// no extra data cost, and the modem is off the entire time.
const uint16_t AVG_WINDOWS = 31;

// --- Reporting cadence (continuous mode only) ------------------------------
// Ignored when ENABLE_SLEEP_CYCLE is true, where one spectrum publishes per
// wake. In continuous mode a spectrum publishes every REPORT_INTERVAL_MS.
const uint32_t REPORT_INTERVAL_MS = 60000;
...
// Cellular association takes far longer than WiFi -- tens of seconds on a good
// signal, minutes at the edge of coverage. Must fit inside AWAKE_TIME_MS with
// room for sampling.
const unsigned long CONNECT_TIMEOUT = 3UL * 60UL * 1000UL;   // 3 minutes
...
float acWindows;   // FFT windows averaged (QC)
float acDropped;   // Windows rejected for bad sample timing (QC)
float acRssi;      // Cellular signal strength, dBm (QC / siting)
...
// Fills sampleBuf with FFT_SIZE samples. Returns false if the achieved rate
// drifted outside tolerance, in which case the window should be discarded.
bool captureWindow(float *achievedRateHz) {
    const uint32_t periodUs = 1000000UL / SAMPLE_RATE_HZ;
...
// binHz is derived from the MEASURED sample rate, not the nominal one.
static void computeBands(float *bandDb, float *bandLinear, float binHz,
                         float nyquistHz) {
    for (uint8_t b = 0; b < N_BANDS; b++) {
        float loHz = BAND_EDGES_HZ[b];
...
void assignBandVariables(const float *bandDb, float ndsi) {
    ...
    if (Cellular.ready()) {
        ...
        acRssi = sig.getStrengthValue();        // dBm
    } else {
        acRssi = 0.0f;
    }
}
...
// Bring the modem up if it is currently down. No-op if already connected.
void radioUp() {
    if (Particle.connected()) return;
    Serial.println("Powering up cellular modem...");
    Cellular.on();
    Cellular.connect();
    Particle.connect();
}
...
void publishAcousticData() {
    ...
    if (!Particle.connected()) {
        Serial.printlnf("Waiting for cellular cloud connection (up to %lu seconds)...",
                        CONNECT_TIMEOUT / 1000);
        waitFor(Particle.connected, CONNECT_TIMEOUT);
    }

    if (Particle.connected()) {
        // RSSI is only meaningful once connected, so refresh it here.
        CellularSignal sig = Cellular.RSSI();
        acRssi = sig.getStrengthValue();
        ...
        Serial.println("Publishing acoustic band data via cellular...");
        Serial.println(msg);
        ...
        // Cellular needs longer than WiFi to get the packet out before sleep.
        delay(3000);
    } else {
        Serial.println("No cellular connection within timeout - skipping publish this cycle");
    }
}
...
void computeAndPublish() {
    ...
    Serial.printlnf(" NDSI=%.3f rejected=%lu", ndsi, rejectedWindows);
...
void enterSleepMode() {
    ...
    if (KEEP_MODEM_IN_STANDBY && !RADIO_OFF_WHILE_SAMPLING) {
        config.network(NETWORK_INTERFACE_CELLULAR,
                       SystemSleepNetworkFlag::INACTIVE_STANDBY);
        Serial.println("Modem kept in standby through sleep");
    } else {
        Particle.disconnect();
        Cellular.off();
        delay(2000);        // modem needs longer than WiFi to power down cleanly
    }
...
void wakeUp() {
    ...
    // Modem stays down through sampling unless configured otherwise.
    if (!RADIO_OFF_WHILE_SAMPLING && !KEEP_MODEM_IN_STANDBY) {
        radioUp();
    }
    ...
    Serial.printlnf("Sampling (modem %s)...",
                    RADIO_OFF_WHILE_SAMPLING ? "off" : "on");
}
...
void setup() {
    ...
    if (!RADIO_OFF_WHILE_SAMPLING || !ENABLE_SLEEP_CYCLE) {
        Cellular.on();
        Cellular.connect();
        Particle.connect();
    }
    ...
    Serial.printlnf("Soundscape logger (Boron): %lu Hz nominal, %u-pt FFT, "
                    "%.2f Hz bins, %u windows/report (%.2f s) -> event \"%s\"",
                    SAMPLE_RATE_HZ, FFT_SIZE,
                    (float)SAMPLE_RATE_HZ / (float)FFT_SIZE, AVG_WINDOWS,
                    AVG_WINDOWS * (float)FFT_SIZE / SAMPLE_RATE_HZ,
                    acousticEventName);
    if (ENABLE_SLEEP_CYCLE) {
        Serial.printlnf("Duty cycle: sample+publish then %lu s ULP sleep "
                        "(awake capped at %lu s)",
                        SLEEP_TIME_MS / 1000, AWAKE_TIME_MS / 1000);
    } else {
        Serial.println("Duty cycle disabled - running continuously");
    }
}
...
void loop() {
    // Safety cap: if sampling or the modem stalls, sleep anyway rather than
    // burning the whole battery awake.
    if (ENABLE_SLEEP_CYCLE && (millis() - awakeStartTime) >= AWAKE_TIME_MS) {
        Serial.println("Awake time cap reached");
        enterSleepMode();
        return;
    }

    // Still filling the average? Capture another window.
    if (windowsAccumulated < AVG_WINDOWS) {
        float achievedRate = 0.0f;
        if (captureWindow(&achievedRate)) {
            accumulateWindow();
            rateSumHz += achievedRate;
    ...
    // Continuous mode: honor REPORT_INTERVAL_MS between publishes.
    if (lastReport != 0 && (millis() - lastReport) < REPORT_INTERVAL_MS) {
        resetAccumulator();
        return;
    }
 ...
 *
 * PIN CHOICE
 *   MIC_PIN is A5. On the Boron, A6 and A7 are shared with other functions --
 *   if you move the mic, prefer A0-A5.
 *
 * MODEM NOISE
 *   With RADIO_OFF_WHILE_SAMPLING true this is largely handled, but the mic's
 *   analog wire should still be short and, if it runs more than a few cm,
 *   twisted with ground. If you set the flag false, expect the noise floor to
 *   rise and the low bands to pick up modem artifacts.
 *
 * POWER
 *   Cellular is the dominant cost. One connect + publish per 30 minutes is far
 *   cheaper than the Argon version's five. If battery life is still short, the
 *   next lever is SLEEP_TIME_MS, not the sampling parameters -- capturing audio
 *   with the modem off is nearly free by comparison.
 *
 * RAISING AVG_WINDOWS
 *   Because sampling now costs almost nothing (modem down, CPU only), a longer
 *   integration is close to free. 470 windows gives ~30 s of audio per publish
 *   instead of 2 s, with the same one row per half hour. That is a much better
 *   characterization of a site, and the single biggest quality improvement
 *   available in this configuration.
 *
 * IF n_dropped IS HIGH
 *   Windows rejected for timing drift. Likely causes: SAMPLE_RATE_HZ too high
 *   for analogRead() (try 8000 to confirm), or system-thread activity. With
 *   the modem off during sampling this should be near zero -- a nonzero count
 *   here is more suspicious on the Boron than it was on the Argon.
 *
 * ALIGNING WAKES TO THE CLOCK
 *   Fixed durations, so wakes drift relative to wall time. For :00/:30
 *   alignment, replace SLEEP_TIME_MS in enterSleepMode() with a value computed
 *   from Time.minute() and Time.second(). The RTC keeps running through ULP
 *   sleep once the cloud has synced it.
 *
 * CALIBRATION
 *   dBFS is relative. Set the MAX4466 gain trimpot once per board and never
 *   touch it: play a steady 1 kHz tone at a fixed level, raise gain until the
 *   peak sits about 6 dB below clipping, and record the setting.
 *
 * CLIPPING
 *   Not detected here, and a clipped waveform generates broadband harmonic
 *   energy that looks like real signal. Before deployment, check in
 *   accumulateWindow() for raw samples at 0 or 4095 and flag the window.
 *
 * WIND
 *   Outdoors, a foam windscreen is not optional. Without one the 20-250 Hz
 *   bands are mostly wind artifact.
